...
from app.services.profile_service import create_profile, save_profile
...
def ingest(data: RawProfileIn, db: Session = Depends(get_db)):
    ...
    if not normalized_first_name and not normalized_last_name:
        raise HTTPException(
            ...
            detail={
                "field": "name",
                "message": "Name could not be parsed into first and last name",
            },
        )
    ...
    if matching_profile:
        ...
        logger.info(
            f"Merged incoming data into existing profile id={matching_profile.id}"
        )
